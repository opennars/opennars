...
public class Closure extends Procedure {
    ...
    final Object parms;
    final Object body;
    final SchemeContext env;

    /**
     * Make a closure from a parameter list, body, and environment. *
     */
    public Closure(Object parms, Object body, SchemeContext env) {
        this.parms = parms;
        this.env = env;
        this.body = (body instanceof IPair && rest(body) == null)
                ? first(body)
                : cons("begin", body);
    }
    ...
    public Object apply(Scheme interpreter, Object args) {
        return interpreter.eval(body, new SchemeContext(parms, args, env));
    }
}
