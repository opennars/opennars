...
import static nars.util.data.sexpression.Pair.*;
...
public class Primitive extends Procedure {
    ...
    public static SchemeContext installPrimitives(SchemeContext env) {
    ...
    public Object apply(Scheme interp, Object args) {
        ...
        switch (idNumber) {
            ...
            case PAIRQ:
                return truth(x instanceof IPair);
            case LISTQ:
    ...
    public static char[] stringAppend(Object args) {
        ...
        for (; args instanceof IPair; args = rest(args)) {
    ...
    public static Object numCompare(Object args, char op) {
        while (rest(args) instanceof IPair) {
    ...
    static boolean isList(Object x) {
        ...
        for (; ; ) {
            ...
            if (slow == rest(fast) || !(fast instanceof IPair)
                    || !(slow instanceof IPair)) return false;
            slow = rest(slow);
            fast = rest(fast);
            if (fast == null) return true;
            if (!(fast instanceof IPair)) return false;
            fast = rest(fast);
        }
    }
    ...
    static Object append2(Object x, Object y) {
        if (x instanceof IPair) return cons(first(x), append2(rest(x), y));
        else return y;
    }
     ...
     * and return that at the end.  Otherwise, just return null. *
     */
    static IPair map(Procedure proc, Object args, Scheme interp, IPair result) {
        IPair accum = result;
        if (rest(args) == null) {
            args = first(args);
            while (args instanceof IPair) {
