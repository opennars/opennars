...
public class Scheme extends SchemeUtils {
    ...
    final InputPort input = new InputPort(System.in);
    final PrintWriter output = new PrintWriter(System.out, true);
    final SchemeContext globalEnvironment = new SchemeContext();
     ...
     */
    public Object eval(Object x, SchemeContext env) {
        ...
        while (true) {
            ...
            if (x instanceof String) {         // VARIABLE
            ...
            } else if (!(x instanceof IPair)) { // CONSTANT
                return x;
            } else {
                Object fn = first(x);
                Object args = rest(x);
                ...
                if (fn == "quote") {             // QUOTE
                    ...
                    if (first(args) instanceof IPair)
                    ...
                    if (fn instanceof Macro) {          // (MACRO CALL)
                        x = ((Macro) fn).expand(this, (IPair) x, args);
                    } else if (fn instanceof Closure) { // (CLOSURE CALL)
                        Closure f = (Closure) fn;
                        x = f.body;
                        env = new SchemeContext(f.parms, evalList(args, env), f.env);
     ...
     */
    IPair evalList(Object list, SchemeContext env) {
        if (list == null)
            return null;
        else if (!(list instanceof IPair)) {
     ...
     */
    Object reduceCond(Object clauses, SchemeContext env) {
        Object result = null;
        for (; ; ) {
