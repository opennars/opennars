        ...
        for(final Task c : current_tasks) { //a =/> b or (&/ a1...an) =/> b
            ...
            if(matched && lastEvents.size()>args.length-off) { 
                ...
                if(right_in_time && Variables.hasSubstitute(Symbols.VAR_INDEPENDENT,imp.getPredicate(),lastEvents.get(args.length-off).sentence.term)) { //it matched and same consequence, so positive evidence
                    ...
                    sentenceMakeParameters.term = c.sentence.term.clone();
                    sentenceMakeParameters.punctuation = Symbols.JUDGMENT_MARK;
                    sentenceMakeParameters.truth = new TruthValue(1.0f,Parameters.DEFAULT_JUDGMENT_CONFIDENCE);
                    sentenceMakeParameters.stamp = new Stamp(nal.memory);
                    Sentence s2 = Sentence.makeByTermPunctuationTruthStampNormalize(sentenceMakeParameters);
                   ...
                   // c.sentence.truth=TruthFunctions.revision(c.sentence.truth, new TruthValue(0.0f,Parameters.DEFAULT_JUDGMENT_CONFIDENCE));
                    ...
                    sentenceMakeParameters.term = c.sentence.term.clone();
                    sentenceMakeParameters.punctuation = Symbols.JUDGMENT_MARK;
                    sentenceMakeParameters.truth = new TruthValue(0.0f,Parameters.DEFAULT_JUDGMENT_CONFIDENCE);
                    sentenceMakeParameters.stamp = new Stamp(nal.memory);
                    Sentence s2 = Sentence.makeByTermPunctuationTruthStampNormalize(sentenceMakeParameters);
