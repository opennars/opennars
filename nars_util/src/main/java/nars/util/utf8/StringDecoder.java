...
final class StringDecoder {
    ...
    public void appendChars(byte[] source, int offset, int length, Appendable target) throws IOException {
        if (target instanceof StringBuilder) {
            //use the stringbuilder version which is optimized for char[] batch append which isnt part of Appendable
