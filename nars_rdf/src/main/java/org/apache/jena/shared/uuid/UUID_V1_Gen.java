...
public class UUID_V1_Gen implements UUIDFactory
{
    ...
    private static String unparse(UUID_V1 uuid) {
        ...
        StringBuffer sBuff = new StringBuffer(36) ;
        JenaUUID.toHex(sBuff, timeLow, 4) ; //8
        sBuff.append('-') ;
        JenaUUID.toHex(sBuff, timeMid, 2) ; //4
        sBuff.append('-') ;
        JenaUUID.toHex(sBuff, _version << 12 | timeHigh, 2) ; //4
        sBuff.append('-') ;
        JenaUUID.toHex(sBuff, (long)_variant << 14 | clockSeq, 2) ; //4
        sBuff.append('-') ;
        JenaUUID.toHex(sBuff, node, 6) ; //12
