...
public class Generator implements LPAgendaEntry, LPInterpreterContext {
    ...
    protected Set<ConsumerChoicePointFrame> generatingCPs = null;
    
    /** The list of active consumer choice points consuming results from this generator */
    protected Set<ConsumerChoicePointFrame> consumingCPs = null;
    ...
    protected final TriplePattern goal;
...
//    public static final int DEFAULT_DEPTH = 100;
     ...
     */
    public Generator(final LPInterpreter interpreter, TriplePattern goal) {
...
//        }
    ...
    public void notifyBlockedOn(ConsumerChoicePointFrame ccp) {
        ensureGenreatingCPs();
        if (generatingCPs.add(ccp)) {
            checkReadyNeeded = true;
        }
    }

    private void ensureGenreatingCPs() {
        if (generatingCPs == null)
            generatingCPs = new LinkedHashSet();
    }
     ...
     * to complete.
     */
    protected static void postCompletionCheckScan(Iterable<Generator> visited ) {
        for ( Generator g : visited )
        {
            if ( g.completionState == LFlag.LIVE )
            {
                if (g.consumingCPs!=null) {
     ...
     */
    protected void propagateLive(Iterable<Generator> filter) {
