...
// accessors
     ...
     */
    private static int maxVarIndex(Node var, int max) {
...
// parser access
    ...
    */
    ...
    public static List<Rule> parseRules(Parser parser) throws ParserException {
        ...
        List<Rule> ruleset = new ArrayList<>(parser.getRulesPreload());
        while (!finished) {
            try {
                parser.peekToken();
...
// parser support
    ...
    public static class Parser {
         ...
         */
        static boolean isSyntax(String token) {
            if (token.length() == 1) {
                char c = token.charAt(0);
                return (c == '(' || c == ')' || c == '[' || c == ']');
            }
...
//                return Node_RuleVariable.WILD;
         ...
         */
        static Node parseNumber(String lit) {
