  ...
  public static SimpleLogger setLogger(SimpleLogger lg) {
    ...
    */
    ...
    boolean isDefaultNamespace( String uri ) {
        return ns.get(uri) != null && ns.get(uri).isEmpty();
    }
...
//		}
    ...
    private void generateDoctypeDeclaration( Model model, PrintWriter pw )
        ...
        pw.print( "<!DOCTYPE " + rdfRDF +" [" );
            for (Entry<String, String> stringStringEntry : prefixes.entrySet())
            {
                if ( isPredefinedEntityName(stringStringEntry.getKey()) )
                {
                    continue;
                }
                pw.print(
                    newline + "  <!ENTITY " + strForPrefix(stringStringEntry.getKey()) + " '" + Util.substituteEntitiesInEntityValue(
