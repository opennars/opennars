...
public class PropertyImpl extends ResourceImpl implements Property {
    ...
    private static final Pattern ordinalMatch = Pattern.compile("_[0-9]+");

    protected static final Logger logger = LoggerFactory.getLogger(PropertyImpl.class);
    ...
    private int computeOrdinal() {
        String localName = getLocalName();
        if (getNameSpace().equals(RDF.getURI()) && ordinalMatch.matcher(localName).matches())
