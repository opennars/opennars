...
public class OneToManyMap<From, To> implements Map<From, To>
{
    ...
    public Set<Map.Entry<From, To>> entrySet() {
        ...
        for (Map.Entry<From, List<To>> fromListEntry : m_table.entrySet())
        {
            List<To> values = fromListEntry.getValue();

            // add each key-value pair to the result set
            for ( ListIterator<To> e1 = values.listIterator(); e1.hasNext(); )
            {
                s.add( new Entry<>(fromListEntry.getKey(), e1.next() ) );
            }
        }
    ...
    public void putAll( Map<? extends From, ? extends To> m ) {
        ...
        for (Map.Entry<? extends From, ? extends To> entry : m.entrySet())
        {
            if ( many )
            {
                ...
                Iterator<? extends To> j = X.getAll(entry.getKey());

                for (; j.hasNext(); )
                {
                    put(entry.getKey(), j.next() );
                }
            }
            else
            {
    ...
    public int size() {
        ...
        for (Map.Entry<From, List<To>> fromListEntry : m_table.entrySet())
        {
            size += fromListEntry.getValue().size();
        }
        return size;
    }
    ...
    public Collection<To> values() {
        ...
        for (Map.Entry<From, List<To>> fromListEntry : m_table.entrySet())
        {
            s.addAll(fromListEntry.getValue());
        }
        return s;
    }
