    ...
    private void potentialReaction(final DerivationContext nal, Sentence projectedGoal, final Task task) {
        try{
            ...
            if(bestop != null && bestop_truthexp > memory.param.decisionThreshold.get() /*&& Math.random() < bestop_truthexp */) {
                ...
                sentenceMakeParameters.term = bestop;
                sentenceMakeParameters.punctuation = Symbols.JUDGMENT_MARK;
                sentenceMakeParameters.truth = bestop_truth;
                sentenceMakeParameters.stamp = projectedGoal.stamp;
                Sentence<Operation> createdSentence = Sentence.makeByTermPunctuationTruthStampNormalize(sentenceMakeParameters);
