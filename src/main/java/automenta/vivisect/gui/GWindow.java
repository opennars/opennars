...
//		papplet.setMinimumSize(new Dimension(papplet.appWidth, papplet.appHeight));
		...
		add((Component) papplet.getSurface(), BorderLayout.CENTER);
 ...
 of method in GUI reference)
	...
	public void setBackground(PImage image){
		...
		papplet.setSize(image.width, image.height);

		Component cmp = (Component) papplet.getSurface();
		cmp.setPreferredSize(new Dimension(papplet.appWidth, papplet.appHeight));
		cmp.setMinimumSize(new Dimension(papplet.appWidth, papplet.appHeight));
		pack();
		super.setResizable(false);
