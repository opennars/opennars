...
import nars.prolog.AbstractSocket;
import nars.prolog.Client_Socket;
import nars.prolog.Datagram_Socket;
import nars.prolog.Int;
import nars.prolog.Library;
import nars.prolog.Prolog;
import nars.prolog.PrologError;
import nars.prolog.Server_Socket;
...
public class SocketLibrary extends Library implements ISocketLib {
	...
	{
		if (!(Socket.getTerm() instanceof nars.prolog.Var)) { // Socket has to be a variable
...
public boolean udp_socket_close_1(Term Socket) throws PrologError {
	if (Socket.getTerm() instanceof nars.prolog.Var) { 			
		throw PrologError.instantiation_error(engine.getEngineManager(), 1);
	}
...
public boolean tcp_socket_server_accept_3(Term ServerSock, Term Client_Addr, Term Client_Slave_Socket) throws PrologError {
	...
	if (ServerSock.getTerm() instanceof nars.prolog.Var) { 	// ServerSock has to be bound
		throw PrologError.instantiation_error(engine.getEngineManager(), 1);
	}
...
public boolean tcp_socket_client_open_2(Struct Address, Term SocketTerm) throws PrologError {
	if (!(SocketTerm.getTerm() instanceof nars.prolog.Var)) { // Socket has to be a variable
...
public synchronized boolean tcp_socket_server_close_1(Term serverSocket) throws PrologError {
	if (serverSocket.getTerm() instanceof nars.prolog.Var) { 			// serverSocket has to be bound
		throw PrologError.instantiation_error(engine.getEngineManager(), 1);
	}
...
public boolean write_to_socket_2(Term Socket, Term Msg) throws PrologError {
	if (Socket.getTerm() instanceof nars.prolog.Var) { // Socket has to be bound
		throw PrologError.instantiation_error(engine.getEngineManager(), 1);
	}
	if (((AbstractSocket) Socket.getTerm()).isServerSocket()) { // Only Client_Sockets can send data
		throw PrologError.instantiation_error(engine.getEngineManager(), 1);
	}
	if (Msg.getTerm() instanceof nars.prolog.Var) { // Record has to be bound
		throw PrologError.instantiation_error(engine.getEngineManager(), 2);
...
public boolean read_from_socket_3(Term Socket, Term Msg, Struct Options) throws PrologError {
	...
	if (!(Msg.getTerm() instanceof nars.prolog.Var)) { // Message has to be a variable
		throw PrologError.instantiation_error(engine.getEngineManager(), 2);
	}
...
public boolean getAddress_2(Term sock, Term addr) throws PrologError {
	if (sock.getTerm() instanceof nars.prolog.Var) { // Socket has to be bound
		throw PrologError.instantiation_error(engine.getEngineManager(), 1);
	}
