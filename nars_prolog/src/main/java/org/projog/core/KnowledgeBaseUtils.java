...
/**
 * Helper methods for performing common tasks on {@link KB} instances.
 */
public final class KnowledgeBaseUtils {
    ...
    */
   public static void bootstrap(KB kb) {
...
//   }
    ...
    */
   public static List<PredicateKey> getPredicateKeysByName(KB kb, String predicateName) {
      List<PredicateKey> matchingKeys = new ArrayList<>();
      for (PredicateKey key : kb.getDefined().keySet()) {
    ...
    */
   public static Predicate getPredicate(KB kb, PTerm t) {
      return kb.getPredicateFactory(t).getPredicate(t.terms());
   }
   ...
   public static boolean isQuestionOrDirectiveFunctionCall(PTerm t) {
      return t.type() == PrologOperator.STRUCTURE && t.length() == 1 && (QUESTION_PREDICATE_NAME.equals(t.getName()) || IMPLICATION_PREDICATE_NAME.equals(t.getName()));
   }
   ...
   private static boolean isConjunctionWithSingleResult(KB kb, PTerm antecedant) {
      PTerm[] functions = toArrayOfConjunctions(antecedant);
      return isAllSingleAnswerFunctions(kb, functions);
   }

   private static boolean isAllSingleAnswerFunctions(KB kb, PTerm[] functions) {
   ...
   public static boolean isConjunction(PTerm t) {
      ...
      return t.type() == PrologOperator.STRUCTURE && CONJUNCTION_PREDICATE_NAME.equals(t.getName()) && t.terms().length == 2;
   }

   public static ProjogEventsObservable getProjogEventsObservable(KB kb) {
      return getServiceLocator(kb).getInstance(ProjogEventsObservable.class);
   }

   public static ProjogProperties getProjogProperties(KB kb) {
   ...
   public static TermFormatter getTermFormatter(KB kb) {
      return getServiceLocator(kb).getInstance(TermFormatter.class);
   }

   public static SpyPoints getSpyPoints(KB kb) {
      return getServiceLocator(kb).getInstance(SpyPoints.class);
   }

   public static FileHandles getFileHandles(KB kb) {
      return getServiceLocator(kb).getInstance(FileHandles.class);
   }

   public static Calculatables getCalculatables(KB kb) {
