...
import nars.tuprolog.interfaces.IParser;
 ...
 * <p/>
 * BNF part 2: Parser term ::= exprA(1200) exprA(n) ::= exprB(n) { op(yfx,n)
 * exprA(n-1) | op(yf,n) }* exprB(n) ::= exprC(n-1) { op(xfx,n) exprA(n-1) |
 * op(xfy,n) exprA(n) | op(xf,n) }* // exprC is called parseLeftSide in the code
 * exprC(n) ::= '-' integer | '-' float | op( fx,n ) exprA(n-1) | op( fy,n )
 * exprA(n) | exprA(n) exprA(0) ::= integer | float | atom | variable | atom'('
...
public class Parser implements /*Castagna 06/2011*/ IParser,/**/ Serializable, Iterable<PTerm> {
    ...
    public PTerm nextTerm(boolean endNeeded) throws InvalidTermException {
        try {
            ...
            if (endNeeded && tokenizer.readToken().getType() != Tokenizer.END) /*Castagna 06/2011*/ //throw new InvalidTermException("The term " + term + " is not ended with a period.");
            {
    ...
    private IdentifiedTerm exprB(int maxPriority, boolean commaIsEndMarker) throws InvalidTermException, IOException {
        ...
        for (; operator.isOperator(commaIsEndMarker); operator = tokenizer.readToken()) {
            ...
            if (XFX >= XFY && XFX >= XF && XFX >= left.priority) {     //XFX has priority
                ...
                if (found != null) {
                    /*Castagna 06/2011*/
					//Struct xfx = new Struct(operate.seq, left.result, found.result);
            ...
            if (XFY >= XF && XFY >= left.priority) {           //XFY has priority, or XFX has failed
                ...
                if (found != null) {
                    /*Castagna 06/2011*/
					//Struct xfy = new Struct(operate.seq, left.result, found.result);
            ...
            /*Castagna 06/2011*/ //return new IdentifiedTerm(XF, new Struct(operate.seq, left.result));
            {
     ...
     *
     * @param commaIsEndMarker used when the leftside is part of and argument
     * list of expressions
     * @param maxPriority operators with a higher priority than this will
     * effectivly end the expression
    ...
    /**
     * exprA(0) ::= integer | float | variable | atom | atom( exprA(1200) { ,
     * exprA(1200) }* ) | '[' exprA(1200) { , exprA(1200) }* [ | exprA(1200) ]
