...
public abstract class DefaultCompound<T extends Term> extends TermVector<T> implements Compound<T> {
    ...
    public int compareTo(final Object o) {
        ...
        //HACK
        final DefaultCompound c = (DefaultCompound) o;
...
//        }
        ...
        int subt = 1 << op().ordinal();

        int contentHash = newContentHash(subt, getHashSeed());
...
//     */
    ...
    public boolean equals(final Object that) {
        if (this == that)
            return true;
        if (!(that instanceof DefaultCompound)) return false;
