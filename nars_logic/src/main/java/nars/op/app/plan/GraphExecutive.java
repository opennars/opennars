...
import static nars.op.app.plan.MultipleExecutionManager.isPlanTerm;
   ...
   public static float getEffectiveExpectation(final Memory memory, final Term current) {        
    ...
    public class ParticlePlan implements Comparable<ParticlePlan> {
        ...
        public ParticlePlan(Cause[] path, List<Term> sequence, double pathScore, double distance) {
            ...
            budget = new Budget(1.0f, Global.DEFAULT_GOAL_DURABILITY,
...
//            }
            ...
            this.goal = goal;
            this.solution = new Task(new Sentence((Implication)imp, punctuation, truth, stamp), budget, goal);
            return solution;
        }
