...
import nars.storage.Memory.MemoryModel;
...
abstract public class NARBuilder extends Parameters implements ConceptBuilder {
    ...
    abstract public MemoryModel newMemoryModel(Param p);
    
    public NAR build() {
        Param p = newParam();
        Operator[] operators = DefaultOperators.get();
        Memory m = new Memory(p, newMemoryModel(p), newNovelTaskBag(p), this, operators);
