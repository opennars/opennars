...
public final class TruthFunctions extends UtilityFunctions {
     ...
     * @return Truth value of the conclusion
     */
    static final TruthValue desireDed(final TruthValue v1, final TruthValue v2) {
        final float f1 = v1.getFrequency();
        final float f2 = v2.getFrequency();
        final float c1 = v1.getConfidence();
        final float c2 = v2.getConfidence();
    ...
    public static final float temporalProjection(long sourceTime, long targetTime, long currentTime) {
        return Math.abs(sourceTime - targetTime) / (float) (Math.abs(sourceTime - currentTime) + Math.abs(targetTime - currentTime));
    }
}
